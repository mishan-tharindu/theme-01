...
final class FLBuilderFontAwesome {
	...
	static function get_fa_data() {
		...
		if ( ! $kit ) {
			...
			$result = wp_remote_get( sprintf( 'https://kit.fontawesome.com/%s.js', $kit->data->me->kit->token ) );
			if ( is_array( $result ) && 403 === $result['response']['code'] ) {
				$error_txt = sprintf( 'Not allowed https://kit.fontawesome.com/%s.js', $kit->data->me->kit->token );
				if ( isset( $result['body'] ) && $error_txt === $result['body'] ) {
					$data['kit403'] = array(
